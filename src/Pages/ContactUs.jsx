...
import FormInput, { FromInput1, InputError } from "../components/FormInput";
...
export default function ContactUs() {
  ...
  const onSubmit = async (inputs) => {
    ...
    if (status) {
      ...
      setError(
        "File",
        { type: "focus", message: "هنالك مشكلة ف الملف الملحق!" },
        { shouldFocus: true }
      );
    }
  };
  ...
  const handleFileInputChange = (event) => {
    ...
    if (file) {
      ...
      if (fileType !== "image/png" && fileType !== "image/jpeg") {
        console.log("Only PNG and JPEG files are allowed.");
        setError(
          "File",
          { type: "focus", message: "يرجي ادخال صورة من صيغة PNG او JPG فقط" },
          { shouldFocus: true }
        );
        return;
      }
      ...
      if (fileSizeInMB > 5) {
        ...
        setError(
          "File",
          { type: "focus", message: "يرجى ادخال صورة لا تتخطي ال 5MB" },
          { shouldFocus: true }
        );
        return;
      }
